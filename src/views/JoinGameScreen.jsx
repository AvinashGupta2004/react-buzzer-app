...
function JoinGameScreen(){
    ...
    const [players,setPlayers] = useState(location.state?.roomState?.players || []);
    const [isGameActive,setIsGameActive] = useState(location.state?.roomState?.isGameActive || false);
    const [buzzes,setBuzzes] = useState(location.state?.roomState?.buzzes || []);
    ...
    useEffect(() => {
        ...
        if (location.state?.roomState) {
            setPlayers([...location.state.roomState.players]);
            setBuzzes([...location.state.roomState.buzzes]);
            setIsGameActive(location.state.roomState.isGameActive);
        }
        ...
        <main className={`h-full p-6`}>
            ...
            <section>
                ...
                <article className={``}>
                    ...
                    <figure
                        className={`w-full lg:w-[60%] mt-4 mx-auto flex flex-col justify-center gap-3 overflow-y-auto`}>
                        {
